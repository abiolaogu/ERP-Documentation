...
- Primary URL: http://localhost:3012
...
- PeopleForce feature data persisted through ERP-DBaaS Hasura via module API routes:
  - `/api/peopleforce/bootstrap`
  - `/api/peopleforce/preferences`
  - `/api/peopleforce/notifications`
  - `/api/peopleforce/events`
  - `/api/peopleforce/requests`
- Required Hasura schema bootstrap SQL:
  `src/features/peopleforce/server/hasura-schema.sql`
