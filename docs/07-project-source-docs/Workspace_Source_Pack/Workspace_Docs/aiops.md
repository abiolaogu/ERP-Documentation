...
|--------|-------|-------------|
| Health heartbeat | `[env].[org].erp.workspace.health` | 30s interval liveness/readiness |
| Error events | `[env].[org].erp.workspace.errors` | Application-level errors |
| Metric snapshots | `[env].[org].erp.workspace.metrics` | Key business/operational metrics |
