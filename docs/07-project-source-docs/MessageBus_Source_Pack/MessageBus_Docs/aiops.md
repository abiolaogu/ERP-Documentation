...
|--------|-------|-------------|
| Health heartbeat | `[env].[org].erp.messagebus.health` | 30s interval liveness/readiness |
| Error events | `[env].[org].erp.messagebus.errors` | Application-level errors |
| Metric snapshots | `[env].[org].erp.messagebus.metrics` | Key business/operational metrics |
