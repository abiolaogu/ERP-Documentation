...
|--------|-------|-------------|
| Health heartbeat | `[env].[org].erp.bi.health` | 30s interval liveness/readiness |
| Error events | `[env].[org].erp.bi.errors` | Application-level errors |
| Metric snapshots | `[env].[org].erp.bi.metrics` | Key business/operational metrics |
