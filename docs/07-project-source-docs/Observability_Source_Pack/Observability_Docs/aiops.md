...
|--------|-------|-------------|
| Health heartbeat | `[env].[org].erp.observability.health` | 30s interval liveness/readiness |
| Error events | `[env].[org].erp.observability.errors` | Application-level errors |
| Metric snapshots | `[env].[org].erp.observability.metrics` | Key business/operational metrics |
