...
|--------|-------|-------------|
| Health heartbeat | `[env].[org].erp.ai.health` | 30s interval liveness/readiness |
| Error events | `[env].[org].erp.ai.errors` | Application-level errors |
| Metric snapshots | `[env].[org].erp.ai.metrics` | Key business/operational metrics |
