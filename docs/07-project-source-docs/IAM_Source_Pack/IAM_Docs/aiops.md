...
|--------|-------|-------------|
| Health heartbeat | `[env].[org].erp.iam.health` | 30s interval liveness/readiness |
| Error events | `[env].[org].erp.iam.errors` | Application-level errors |
| Metric snapshots | `[env].[org].erp.iam.metrics` | Key business/operational metrics |
