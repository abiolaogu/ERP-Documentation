...
```
Design a control plane view for ERP-SCM that shows:
...
```
Generate a role-aware workspace for ERP-SCM with:
...
```
Design a full incident-response flow for ERP-SCM:
...
```
Create mobile and tablet executive dashboards for ERP-SCM:
...
```
Create a Make scenario for ERP-SCM that:
...
```
Generate a Make scenario for tenant onboarding in ERP-SCM:
