...
```
Design a control plane view for ERP-IAM that shows:
...
```
Generate a role-aware workspace for ERP-IAM with:
...
```
Design a full incident-response flow for ERP-IAM:
...
```
Create mobile and tablet executive dashboards for ERP-IAM:
...
```
Create a Make scenario for ERP-IAM that:
...
```
Generate a Make scenario for tenant onboarding in ERP-IAM:
